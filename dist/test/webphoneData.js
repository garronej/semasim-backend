...
const evt_1 = require("../../../frontend/shared/node_modules/evt");
...
const evtSendRequest = new evt_1.Evt();
const evtWdChatCheckedFromOtherDevice = new evt_1.Evt();
async function testApiCall(client, runApiCall, expect) {
    const boundTo = evt_1.Evt.newCtx();
    ...
    try {
        tools_1.assert(expect.requestSent === wasRequestSent);
    }
    catch (error) {
        console.log({ expect });
        throw error;
    ...
    if (expect.otherUaUpdateEvent !== undefined) {
        tools_1.assert(expect.otherUaUpdateEvent === wasChangeNotifiedToOtherUas);
    }
    return out;
}
...
;
function getIsolatedWdApiCallerForSpecificSimFactory(params) {
    ...
    return function getIsolatedWdApiCallerForSpecificSim(platform, uaInstanceId) {
        ...
        const handlers = webphoneData_1.getHandlers(impl_1.getApi((() => {
            ...
            "wd_notifyActionFromOtherUa": ({ methodNameAndParams, uas }) => Promise.all(uas
                ...
                const handlerCount = evtWdChatCheckedFromOtherDevice
                    .getHandlers()
                    .filter(({ op }) => !!evtWdChatCheckedFromOtherDevice
        ...
        return webphoneData_2.getWdApiFactory({
            "sendRequest": (methodName, params) => {
                var _a;
                const { handler, sanityCheck } = handlers[methodName];
                tools_1.assert((_a = sanityCheck === null || sanityCheck === void 0 ? void 0 : sanityCheck(params)) !== null && _a !== void 0 ? _a : true, `SanityCheck failed ${methodName}, ${JSON.stringify(params, null, 2)}`);
...
async function runTests() {
    ...
    for (let i = 0; i <= 5; i++) {
        ...
        testApiCall(androidClient1, () => androidClient1.frontendApi.updateChatContactInfos({
            wdChat,
            contactName,
            contactIndexInSim
        }), { "requestSent": false });
    ...
    tools_1.assert(androidClient1.wdChats.length === 0);
    {
        ...
        await Promise.all((new Array(5))
            ...
            await Promise.all(androidClients.map(async (client) => {
                ...
                const { wdMessage } = await (() => {
                    ...
                    return client.wdEvts.evtWdMessage.waitFor(({ wdChat: wdChat_, wdMessage }) => (wdChat_ === wdChat &&
                        wdMessagesBefore.indexOf(wdMessage) < 0 &&
                        wdMessage.direction === "INCOMING" &&
                        wdMessage.time === time), 10000);
                })();
                tools_1.assert(typeof wdMessage.ref === "string");
                tools_1.assert(wdChat.messages.indexOf(wdMessage) >= 0);
    ...
    tools_1.assert(androidClient1.wdChats.length === 0);
    {
        ...
        await assertCheckPass();
        const test = (client, expect) => testApiCall(client, () => client.frontendApi.newMessage({
    ...
    tools_1.assert(androidClient1.wdChats.length === 0);
    {
        ...
        const messageRef = await (async () => {
            ...
            const { wdMessage } = await (() => {
                const wdMessagesBefore = [...wdChat.messages];
                return androidClient1.wdEvts.evtWdMessage.waitFor(({ wdChat: wdChat_, wdMessage }) => (wdChat_ === wdChat &&
                    wdMessagesBefore.indexOf(wdMessage) < 0 &&
        ...
        for (let i = 0; i < snapshotClients.length; i++) {
            ...
            }
            {
                ...
                switch (i) {
                    case 0:
                    case 1:
                        tools_1.assert(wdMessage === undefined);
                        break;
                    case 2:
                        tools_1.assert(!!wdMessage &&
                            wdMessage.direction === "OUTGOING" &&
            ...
            await testApiCall(client, () => client.frontendApi.notifyStatusReportReceived({
                wdChat,
                "bundledData": bundledDataStatusReport
            }), (() => {
                switch (i) {
    ...
    tools_1.assert(androidClient1.wdChats.length === 0);
    {
        ...
        tools_1.assert(types.wd.Chat.getUnreadMessagesCount(wdChat) === 0);
        ...
        const wdMessageOutgoingRef = await (async () => {
            ...
            const [wdMessage, ...othersWdMessages] = (await Promise.all([
                androidClient1.wdEvts.evtWdMessage.attachOnce(({ wdChat: wdChat_, wdMessage }) => (wdChat_ === wdChat &&
                    wdMessagesBefore.indexOf(wdMessage) < 0 &&
                    wdMessage.direction === "OUTGOING" &&
                    wdMessage.status === "PENDING"), 10000, () => tools_1.assert(types.wd.Chat.getUnreadMessagesCount(wdChat) === 0)),
                androidClient1.wdEvts.evtWdMessage.attachOnce(({ wdChat: wdChat_, wdMessage }) => (wdChat_ === wdChat &&
                    wdMessage.direction === "OUTGOING" &&
                    wdMessage.status === "SEND REPORT RECEIVED"), 10000, () => tools_1.assert(types.wd.Chat.getUnreadMessagesCount(wdChat) === 0)),
                androidClient1.wdEvts.evtWdMessage.attachOnce(({ wdChat: wdChat_, wdMessage }) => (wdChat_ === wdChat &&
                    wdMessage.direction === "OUTGOING" &&
                    wdMessage.status === "STATUS REPORT RECEIVED"), 10000, () => tools_1.assert(types.wd.Chat.getUnreadMessagesCount(wdChat) === 1))
        ...
        tools_1.assert(types.wd.Chat.getUnreadMessagesCount(wdChat) === 2);
        tools_1.assert((() => {
        ...
        tools_1.assert(types.wd.Chat.getUnreadMessagesCount(wdChat) === 0);
        tools_1.assert(wdChat.refOfLastMessageSeen
    ...
    tools_1.assert(androidClient1.wdChats.length === 0);
    {
        ...
        await Promise.all((new Array(maxMessageCountByChat * 3))
            ...
            .reverse()
            .map(async (bundledData) => androidClient1.frontendApi.newMessage({
            wdChat,
            "type": "SERVER TO CLIENT",
            bundledData
        })));
        tools_1.assert(wdChat.messages.length === messageCount &&
        ...
        tools_1.assert(wdChat.messages.length === maxMessageCountByChat);
        {
            const n = 10;
            await androidClient1.frontendApi.fetchOlderMessages({ wdChat, "maxMessageCount": n });
            tools_1.assert(wdChat.messages.length === maxMessageCountByChat + n);
        }
...
(async () => {
    ...
    tools_1.assert(deploy_1.deploy.getEnv() === "DEV", "You DO NOT want to run DB tests in prod");
    await runTests();
    process.exit(0);
})();

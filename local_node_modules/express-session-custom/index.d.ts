...
declare global {
  namespace Express {
    interface Request {
      session?: Session;
      sessionID?: string;
      connect_sid?: string; /** Value exactly as in cookie, accessor present when sessionID present, never throw */
    }
...
interface SessionMiddleware {
  /** connect_sid can be explicitly  passed if retrieved from header (or some other way ) */
  (req: express.Request, res: express.Response, next: express.NextFunction, connect_sid?: string): any;
  getReadonlySession: (connect_sid: string) => Promise<{ session: Express.Session; touch: ()=> Promise<boolean> }>;
}

declare function session(options?: session.SessionOptions): SessionMiddleware;
